...
class TestPolicySchema(unittest.TestCase):
    def test_validate_policy_rejects_unknown_keys(self) -> None:
        with self.assertRaises(ValueError):
            merge_policy(DEFAULT_POLICY, {"unknown_field": True})

    def test_validate_policy_rejects_invalid_threshold_order(self) -> None:
        bad = dict(DEFAULT_POLICY)
        bad["sandbox_total_risk_gte"] = 99
        bad["block_total_risk_gte"] = 50
        with self.assertRaises(ValueError):
            validate_policy(bad)

    def test_validate_policy_accepts_default(self) -> None:
        validated = validate_policy(dict(DEFAULT_POLICY))
